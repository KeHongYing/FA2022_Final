...
public:
          ...
          periodPerDay(periodPerDay)
    {
        ...
        downScale = 1 / upScale;
        vasicekRate = getVasicekRate(periodRate, K, periodRate, annualVolatility, periodPerDay * maturityDay, 42);
    ...
    vector<double> getVasicekRate(double r0, double K, double theta, double sigma, int N, int seed)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            double dr = K * (theta - rates.back()) * dt + sigma * sqrt(dt) * distribution(generator);
            rates.push_back(rates.back() + dr);

            // cout << n << " " << rates.back() << endl;
        }
